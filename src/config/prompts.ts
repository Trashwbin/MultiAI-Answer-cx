...
const SYSTEM_PROMPT_BASE = `你是一个精准的考试答题助手。你的任务是根据给定的题目列表，以 JSON 格式返回答案。
...
7. confidence 为你对答案的把握程度 (0.0-1.0)
8. 只输出 JSON，不要输出任何 markdown 标记或额外文字`;
...
export function buildPrompt(
  ...
  const parts: string[] = [SYSTEM_PROMPT_BASE, FEW_SHOT_EXAMPLE];

  if (mode === 'analysis') {
    parts.push(ANALYSIS_ADDENDUM);
  }
